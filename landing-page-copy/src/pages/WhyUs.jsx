  ...
  return (
    <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
      {VALUE_PROPOSITIONS.map((vp) => (
        <Card key={vp.title} className="transition-all hover:-translate-y-1 hover:shadow-lift">
            ...
            {vp.capabilities && (
              <div className="mt-3 flex flex-wrap gap-1.5">
                {vp.capabilities.map((c) => (
                  <span key={c} className="rounded-full border border-slate-200 bg-white px-2 py-0.5 text-[10px] font-medium text-slate-500">{c}</span>
                ))}
              </div>
            )}
          </CardContent>
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      {WHY_CHOOSE_LIST.map((item) => (
        <div key={item.number} className="rounded-2xl bg-gradient-to-br from-brand-700 via-brand-600 to-brand-500 p-6 text-center text-white shadow-lift">
          <span className="mx-auto flex size-10 items-center justify-center rounded-full bg-white/20 font-display text-lg font-bold">{item.number}</span>
  ...
  return (
    <div>
      ...
      <div className="mt-6 grid gap-6 sm:grid-cols-2">
        <Card className="transition-all hover:-translate-y-1 hover:shadow-lift">
