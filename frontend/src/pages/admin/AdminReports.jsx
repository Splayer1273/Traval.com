  ...
  if (isLoading) {
    return (
      <div className="container-x space-y-4 py-8">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{[0, 1, 2, 3].map((i) => <Skeleton key={i} className="h-32 rounded-2xl" />)}</div>
        ...
        <div className="container-x py-8">
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div>
              ...
              <h1 className="mt-1.5 font-display text-3xl font-semibold text-slate-900 sm:text-4xl">Spend reports</h1>
            ...
            <div className="flex flex-wrap gap-2">
              <Button variant="secondary" disabled={!!exporting} onClick={() => downloadCsv('trips')}>
                {exporting === 'trips' ? <Loader2 className="size-4 animate-spin" /> : <Plane className="size-4" />} Export trips CSV
              </Button>
              <Button variant="secondary" disabled={!!exporting} onClick={() => downloadCsv('expenses')}>
                {exporting === 'expenses' ? <Loader2 className="size-4 animate-spin" /> : <Receipt className="size-4" />} Export expenses CSV
              </Button>
            </div>
          </div>
            ...
            <CardContent className="p-5 sm:p-6">
              <div className="mb-5 flex items-center justify-between">
                ...
                <div className="flex items-center gap-3 text-[11px] font-semibold text-slate-500">
                  <span className="flex items-center gap-1.5"><span className="size-2.5 rounded-sm bg-brand-500" /> Trips</span>
                  <span className="flex items-center gap-1.5"><span className="size-2.5 rounded-sm bg-teal-400" /> Claims</span>
                </div>
              </div>
              <div className="flex h-48 items-end gap-2">
                {byMonth.map((m) => (
                  <div key={m.label} className="flex h-full flex-1 flex-col items-center justify-end gap-1.5">
                    <span className="text-[10px] font-bold text-slate-600"><Price amount={m.total} /></span>
                    ...
                    <span className="text-[11px] font-semibold text-slate-500">{m.label.split(' ')[0]}</span>
                  </div>
                ))}
              </div>
            </CardContent>
                ...
                {byDestination.length === 0 ? <p className="text-sm text-slate-500">No trips yet.</p> : (
                  byDestination.slice(0, 6).map((d) => (
                    <div key={d.destination} className="flex items-center gap-3">
                      <span className="w-28 shrink-0 truncate text-sm font-semibold text-slate-700">{d.destination}</span>
                      ...
                      <span className="w-24 shrink-0 text-right text-xs font-bold text-slate-600"><Price amount={d.total} /></span>
                    </div>
                  ))
                )}
              </div>
                ...
                {byCategory.length === 0 ? <p className="text-sm text-slate-500">No claims yet.</p> : (
                  byCategory.map((c) => (
                    <div key={c.category} className="flex items-center gap-3">
                      <span className="w-28 shrink-0 truncate text-sm font-semibold text-slate-700">
                      ...
                      <span className="w-24 shrink-0 text-right text-xs font-bold text-slate-600"><Price amount={c.total} /></span>
                    </div>
                  ))
                )}
              </div>
