  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      {SERVICES_OVERVIEW.items.map((item) => (
        <div key={item.label} className="rounded-2xl bg-gradient-to-br from-brand-700 via-brand-600 to-brand-500 p-6 text-center text-white shadow-lift">
        ...
        {service.options && (
          <div className="mt-4 flex flex-wrap gap-2">
            {service.options.map((o) => (
              <span key={o} className="rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-medium text-slate-600">{o}</span>
            ))}
          </div>
        )}
        ...
        {service.destinations && (
          <div className="mt-4 flex flex-wrap gap-2">
            {service.destinations.map((d) => (
              <span key={d} className="rounded-full border border-brand-200 bg-brand-50 px-3 py-1 text-xs font-medium text-brand-700">{d}</span>
            ))}
          </div>
        )}
        ...
        {service.idealFor && (
          <div className="mt-4 flex flex-wrap gap-2">
            {service.idealFor.map((i) => (
              <span key={i} className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-xs font-medium text-slate-600">{i}</span>
            ))}
          </div>
        )}
    ...
    <div>
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {HOW_IT_WORKS_STEPS.map((step) => (
          <div key={step.step} className="rounded-2xl bg-gradient-to-br from-brand-700 via-brand-600 to-brand-500 p-6 text-center text-white shadow-lift">
            <span className="mx-auto flex size-10 items-center justify-center rounded-full bg-white/20 text-sm font-bold">{step.step}</span>
